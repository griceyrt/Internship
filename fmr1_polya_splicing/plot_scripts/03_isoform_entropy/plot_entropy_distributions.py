#!/usr/bin/env python3
# =============================================================================
# Entropy distribution comparison, WT vs Fmr1 KO
#
# Plots per-gene Shannon entropy of isoform usage (entropy_WT, entropy_KO
# from isoform_proportions_step3.R) as two overlaid distributions, to check
# whether the KO distribution is shifted relative to WT.
#
# entropy_WT and entropy_KO are paired per gene, so the significance test
# is a paired Wilcoxon signed-rank test (nonparametric, since entropy
# values aren't expected to be normally distributed) rather than a t-test.
#
# Run once per TPM threshold (10/20/50).
#
...
# Input tables come from the R pipeline (step1-4), shared with
# plot_isoform_stacked_bars.py, so they stay in isoform_proportions/.
# Outputs of this script get their own folder, entropy_distribution/,
# since it's a separate deliverable from the stacked bars.
...
# Deliberately a different palette family from the stacked bars: this is a
# two-group comparison (WT vs KO across all genes), not per-isoform
# identity within one gene, so a two-color scheme reads more clearly here.
