#!/usr/bin/env Rscript
# =============================================================================
# Isoform proportion stacked-bar figure -- STEP 1 of N
# Author: Gricey
#
# Goal (per Kiran, 2026-08-05): for a set of genes, show WT vs KO isoform
# usage as two stacked bars per gene, one segment per isoform, dropping
# isoforms with TPM == 0 in that condition (confirmed by Gricey's own
# observation of Lies's example figure: same gene showed 2 segments in WT,
# 5 in PerKO -- proof isoforms are dropped per-condition, not always shown).
#
# This step ONLY does: average each condition's 3 replicates, apply the
# TPM>0 filter per condition, and save an inspectable intermediate table.
# Grouping by gene and computing proportions comes in a later step, once
# this looks right.
#
# Run from: fmr1_polya_splicing/
# =============================================================================
...
cat("Next step: group these by gene and compute per-gene isoform proportions.\n")
