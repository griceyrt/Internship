#!/usr/bin/env Rscript
# =============================================================================
# Isoform proportion stacked-bar figure -- STEP 2 of N
# Author: Gricey
#
# Builds on Step 1's avg_tpm_per_condition.tsv: joins in gene_id (from the
# GTF, same tx2gene logic as normalisation_deseq2_oarfish.R), then for each
# gene computes each isoform's share of that gene's total TPM, separately
# for WT and KO, keeping only isoforms present (TPM>0) in that condition --
# this is what makes the WT bar potentially show a different number of
# segments than the KO bar for the same gene.
# =============================================================================
...
# =============================================================================
# BUILD tx2gene FROM GTF (same logic as normalisation_deseq2_oarfish.R,
# now also pulling gene_name so the final figures can show real gene names,
# not just Ensembl IDs)
# =============================================================================
...
# =============================================================================
# ALSO SAVE: how many isoforms are present per gene, per condition -- this
# is exactly the "2 boxes in WT vs 5 in KO" count, useful later for picking
# which genes actually show a change worth plotting
# =============================================================================
...
cat("\nNext step: pick a gene-selection criterion (e.g. significant SUPPA\n")
cat("dPSI, corrected across event types) and build the actual stacked-bar\n")
cat("plots for the selected genes.\n")
