#!/usr/bin/env Rscript
# =============================================================================
# Isoform proportion stacked-bar figure -- STEP 3 of N
# Author: Gricey
#
...
# entropy shifted between WT and KO. This is the primary selection metric
# discussed 2026-08-05 -- purpose-built for "did this gene's isoform usage
# diversify or concentrate", independent of whether DESeq2 or SUPPA flagged
# it (Overlap #1's 22 genes are kept as an annotation/cross-reference here,
# NOT used as an exclusion filter -- entropy shift can happen with no
# change in total expression at all, which Overlap #1 would never catch).
#
...
#
# This step is deliberately just the ranking + distribution look, per
# Gricey's own "start slow" approach -- no cutoff decided yet, no stats
# test yet, just: compute it, look at it, then decide.
# =============================================================================
...
# isoforms) -- needed to check whether extreme entropy shifts are backed by
# real expression, or are just noise from very low read counts (the exact
# "low N" problem Lies flagged: a gene with only a handful of reads can
# show a wildly unstable entropy value purely by chance, with nothing real
...
# =============================================================================
# FLAG GENES PRESENT IN ONLY ONE CONDITION -- these are a DIFFERENT, more
# extreme category (the whole gene switching on/off), not a comparable
# entropy shift, and don't get an entropy_diff since there's nothing to
# subtract from
# =============================================================================
...
cat("\nNext: pick a cutoff from the distribution above, or move to a formal\n")
cat("statistical test on the isoform distributions before finalizing which\n")
cat("genes go into the actual stacked-bar figures.\n")
