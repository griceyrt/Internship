...
# =============================================================================
# Full 6-sample oarfish quantification (COMBINE-lab/oarfish), replacing
# Salmon per the 2026-08-04 decision (Salmon 2.x's own --ont flag says
# "not supported; use oarfish instead", and oarfish is the actively
# maintained, purpose-built long-read quantifier from the same lab).
# Author: Gricey
#
# Validated 2026-08-04 on WT_Rep1: works against the EXISTING Minimap2
# alignment (no --eqx, no realignment needed), just re-sorted by name
# (oarfish requires name/default order, not the coordinate-sorted BAM
# made for Salmon/indexing). Results checked sane: sum(num_reads) matched
# oarfish's own aligned-read count almost exactly, ~22% of transcripts
# non-zero, mitochondrial genes dominating the top (expected pattern).
#
# tximport has native support (type = "oarfish"), so the DESeq2 R script
# only needs a one-line change, no custom output parsing required.
# =============================================================================
