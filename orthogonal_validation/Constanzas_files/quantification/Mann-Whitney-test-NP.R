...
bracket_tick <- 0.1        # <-- EDIT: length of the short vertical lines at each end of the bracket (was 0.2, shortened to 0.1)
...
p <- ggplot() +
  ...
  annotate(
    "text",
    x = 1.5,
    y = y_bracket + 0.15,   # <-- EDIT: vertical offset of the label above the bracket (raise if the 2-line label overlaps the bracket)
  ...
  labs(
    title = "SRSF3 Nucleoplasm levels WT(CT20) vs PERKO",   # <-- EDIT: plot title
    x = "",
...
# =========================
ggsave(
  "SRSF3_Nucleoplasm-WTvsKO-MannWhitney.png",   # <-- EDIT: output filename
  plot = p,
  width = 8,
  height = 6,
  dpi = 300
