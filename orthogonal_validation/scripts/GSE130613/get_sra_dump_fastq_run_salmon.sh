#!/bin/bash
# Adapted from BHARATH_get_sra_dump_fastq_run_salmon.sh for single-end mode
# (GSE130613 is MARS-Seq): fasterq-dump instead of parallel-fastq-dump, -r
# instead of -1/-2, -l A auto-detects -l U (stranded-reverse). Full trim:
# adapter AGATCGGAAGAG + polyA/T/G (G{12} = NextSeq 500 two-color artifact).
# Author: Gricey
#
...
OUTDIR="${BASE_DIR}/results/GSE130613/${1}_salmon_$(date +"%Y-%m-%d")"
LOG="${OUTDIR}/${1}.log"
CORES=7
