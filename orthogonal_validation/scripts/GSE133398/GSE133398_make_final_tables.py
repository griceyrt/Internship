...
reviewer rebuttal. Companion to scripts/GSE133398_make_final_figures.py (siblings,
not dependents -- both read the same upstream pipeline outputs independently).
...
Produces 3 xlsx files:
  ...
  - GSE133398_significant_splicing_events.xlsx (1 tab: significant splicing events)

Author: Gricey
Run from: orthogonal_validation/
    python3 scripts/GSE133398_make_final_tables.py

Nanopore gene-level dedup: genes with more than one significant Nanopore
transcript are represented by the transcript with the lowest padj_per_ko
(same convention as Option B/stageR elsewhere in this pipeline).
"""
...
OUT = os.path.join(BASE, "results", "GSE133398", "tables_GSE133398")
...
for gid, gname in gene_map.items():
    gene_map_rev.setdefault(gname, gid)
