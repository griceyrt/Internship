...
sample count matrix, ready for DESeq2.

htseq-count output (with --additional-attr=gene_name) has 3 columns:
    gene_id    gene_name    count
plus a handful of special summary rows at the end starting with "__"
(__no_feature, __ambiguous, __too_low_aQual, __not_aligned,
__alignment_not_unique) -- these aren't genes, they're QC counters, and are
split out separately here since they're actually useful diagnostics for the
PCR-duplication/multi-mapping question this whole investigation is about
(e.g. a high __alignment_not_unique or __ambiguous count would mean a lot of
reads couldn't be assigned to a single gene).
...
Example:
    python3 combine_htseq_counts.py \
        results/SRP194523_star_htseq_2026-07-16 \
        results/SRP194523_star_htseq_2026-07-16/combined
