"""
Extends transcript boundaries in transcriptome_productivity.gtf to match Asher's
own MARS-seq processing (SI appendix, PNAS 2020): genes/transcripts extended
1000bp toward the 5' edge and 100bp toward the 3' end, to better capture
3'-biased MARS-seq reads that fall just past the currently annotated end.

Approach (adapted for a Salmon/transcript-FASTA pipeline, not genome-interval
counting like Asher's htseq-count): extend the TERMINAL EXON of each transcript
(first and last exon, per strand), so that when the transcript FASTA is
regenerated (e.g. via gffread) from this GTF + the genome FASTA, the extracted
sequence includes the extra flanking bases. Also extends the parent
"transcript" and "gene" lines' coordinates to stay internally consistent.

Handles both annotated (ENSMUST) and novel FLAIR/UUID transcripts -- both get
the same treatment. Only "gene", "transcript", and "exon" feature lines are
touched; CDS/UTR/codon lines are left as-is (gffread's -w/-x FASTA extraction
uses "exon" lines, not CDS, so this is what actually matters for the Salmon
index rebuild).

Bounds are capped at [1, chromosome_length] using the genome .fai index, so
extension never runs off the end of a chromosome/scaffold.

Usage:
    python3 extend_gtf_3utr_5utr.py <input.gtf> <genome.fa.fai> <output.gtf>

Two-pass streaming approach (keeps memory usage low even on the full 900MB
file): pass 1 records each transcript's strand + min/max exon coordinates;
pass 2 rewrites every line, extending exon/transcript/gene boundaries where
they match the transcript's terminal (min or max) coordinate.
"""
import sys
import re
    ...
    with open(gtf_in) as fin, open(gtf_out, "w") as fout:
        for line in fin:
            ...
            if feature in ("exon", "transcript", "gene"):
                m = TX_ID_RE.search(attrs)
                # "gene" lines have no transcript_id -- extend gene lines
                # using ALL of that gene's transcripts' bounds instead
                # (handled by extending only exon/transcript here; gene
                # lines are left untouched since gffread doesn't need them
                # for FASTA extraction, and per-gene extension across
                # multiple transcripts adds ambiguity we don't need to
