#!/usr/bin/env python3
"""
Orthogonal Validation — GSE130613 new vs old pipeline comparison table
Author: Gricey

For Khushi's rebuttal table (per Kiran/Khushi email thread, 2026-07-20):
counts of transcripts/genes tested and significant/DE, for both the
STAR+htseq-count pipeline (NEW, adopted, "solid" per Kiran) and the older
Salmon+tximport+stageR pipeline (OLD, shown to have an inflated log2FC
spread -- see gse130613_marsseq_rework project notes).

Raw per-row data is loaded into the workbook itself so the Summary sheet's
counts are live COUNTIFS formulas, not hardcoded numbers (same convention as
verify_final_numbers.xlsx). Exception: the OLD pipeline's gene-level
(deduplicated) count is computed in Python and documented in a note, since a
robust tie-safe dedup isn't expressible as a simple Excel formula -- see the
note cell on the Summary sheet for the exact method.

Inputs:
  - results/normalisation_star_htseq_original_gtf/deseq2_star_htseq_KO_vs_WT.tsv (NEW, gene-level)
  - results/normalisation/deseq2_KO_vs_WT.tsv (OLD, transcript-level, Salmon+stageR)
...
NEW_FILE = os.path.join(BASE_DIR, "results", "normalisation_star_htseq_original_gtf", "deseq2_star_htseq_KO_vs_WT.tsv")
OLD_FILE = os.path.join(BASE_DIR, "results", "normalisation", "deseq2_KO_vs_WT.tsv")
OUT_PATH = os.path.join(BASE_DIR, "results", "tables", "GSE130613_pipeline_comparison_for_Khushi.xlsx")
