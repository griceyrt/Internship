#!/usr/bin/env python3
"""
Orthogonal Validation — GSE130613 rMATS-turbo splicing results
Author: Gricey

Builds the threshold-comparison deliverable for Bharath/Kiran to decide between,
same pattern as the GSE133398 p<0.3 -> p<0.05 threshold correction:
  - Volcano figure + significance counts at p<0.3, |dPSI|>0.1 (matches the OLD
    SUPPA convention exactly, for an apples-to-apples "same threshold" look)
  - Volcano figure + significance counts at FDR<0.05, |dPSI|>0.1 (the stricter,
    multiple-testing-corrected view)

Input: rMATS-turbo output, results/SRP194523_rmats_2026-07-20/*.MATS.JC.txt
       (JC = junction-counts-only, preferred over JCEC for single-end data,
       see gse130613_marsseq_rework project notes)

Sign convention: rMATS b1=WT, b2=KO, so IncLevelDifference = WT - KO. Negated
here to dPSI = KO - WT, matching every other dPSI figure/table in this project
("dPSI (PerDKO - WT)").
...
# =============================================================================
# STYLE — same block as plot_results.py, for visual consistency across the project
# =============================================================================
...
# rMATS event-type name -> SUPPA-style short code, so we can reuse the same
# EVENT_COLORS palette used everywhere else in the project (step11 etc.)
EVENT_TYPE_MAP = {
    "SE":   "SE",
    "A5SS": "A5",
    "A3SS": "A3",
...
# =============================================================================
# LOAD + COMBINE all 5 event types
# =============================================================================
...
# =============================================================================
# FIGURE 1 — p<0.3 threshold (matches old SUPPA convention exactly)
# =============================================================================
...
# =============================================================================
# TABLE — xlsx with live COUNTIFS summary (matches verify_final_numbers.xlsx
# convention: formulas, not hardcoded totals, so it recalculates if edited)
# =============================================================================
print("\n=== Building xlsx ===")
...
# --- Summary sheet (live COUNTIFS, not hardcoded) ---
...
# Note cell documenting the comparison context + sign convention
