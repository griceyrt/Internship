#!/usr/bin/env python3
"""
Orthogonal Validation — GSE130613 STAR+htseq-count DE figures (final adopted pipeline)
Author: Gricey

Same two figures as the original Salmon pipeline's step13 (DE volcano) and
step14 (gene-level DE concordance vs Nanopore), rebuilt on the ADOPTED
STAR+htseq-count (original GTF) result instead of Salmon+tximport+stageR.

No step15 equivalent: htseq-count is gene-level only (no transcript
resolution, that EM-based isoform assignment is exactly what we moved away
from), so transcript-level concordance isn't reproducible with this pipeline.
Gene-level (step14-style) is the correct, and only, equivalent.

Thresholds (unchanged from the originals):
  - Volcano (step13-style):      padj<0.05, |log2FC|>0.5
  - Concordance (step14-style):  padj<0.05, |log2FC|>1.5 (matches Khushi's
    manuscript threshold, same as the original step14)

Inputs:
  - results/normalisation_star_htseq_original_gtf/deseq2_star_htseq_KO_vs_WT.tsv
    (already gene-level, one row per gene_id, no dedup needed)
  - data/Table3-differential_expressiong_WTvsPerKO.xlsx (Nanopore, DE_all + perKO_sig sheets)
...
# =============================================================================
# STYLE — same block as plot_results.py / crossreference.py
# =============================================================================
...
DESEQ_FILE = os.path.join(BASE_DIR, "results", "normalisation_star_htseq_original_gtf", "deseq2_star_htseq_KO_vs_WT.tsv")
...
# =============================================================================
# LOAD — STAR+htseq DE result (already gene-level)
# =============================================================================
...
# =============================================================================
# STEP 13-style — DE Volcano (padj<0.05, |log2FC|>0.5)
# =============================================================================
...
# =============================================================================
# STEP 14-style — DE concordance vs Nanopore, gene level (padj<0.05, |log2FC|>1.5)
# =============================================================================
