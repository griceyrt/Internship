...
every pipeline version tested in the MARS-seq spread investigation (since the
Zoom with Bharath 2026-07-15):

  1. Original Salmon pipeline           (already has figures: step13/step14 -- not regenerated)
  2. GEO official counts (sanity check)
  3. 3'UTR-extended Salmon pipeline
  4. STAR + htseq-count (extended GTF)
  5. STAR + htseq-count (original GTF)

All comparisons are done at GENE level (one row per gene, representative
transcript = max |log2FC|, same convention as step14_DE_concordance_gene_level.png),
matched against Nanopore's DE_all sheet. GEO's official count matrix is
indexed by gene SYMBOL (not Ensembl ID), so that one matches on mgi_symbol
instead of gene_id -- everything else matches on gene_id directly.

Threshold used throughout: padj < 0.05, |log2FC| > 0.5 (matching the
gene-level threshold used for the original pipeline's step13/14 figures).

Run from: orthogonal_validation/
Outputs to: results/marsseq_sanity_check/investigation_figures/
"""
import os
import pandas as pd
import numpy as np
...
OUT_DIR = "results/marsseq_sanity_check/investigation_figures"
...
# =============================================================================
# Load Nanopore reference (gene-level, deduped)
# =============================================================================
...
# =============================================================================
# 1. GEO official counts (sanity check) -- indexed by gene SYMBOL
# =============================================================================
print("\n=== GEO official counts ===")
geo = pd.read_csv("results/marsseq_sanity_check/deseq2_results_geo_counts.csv", index_col=0)
...
# (volcano already exists: results/marsseq_sanity_check/sanity_check_volcano_geo_counts.png)

# =============================================================================
# 2. 3'UTR-extended Salmon pipeline -- transcript-level, needs gene dedup
# =============================================================================
print("\n=== 3'UTR-extended Salmon ===")
ext = pd.read_csv("results/normalisation_3utr_extended/deseq2_KO_vs_WT.tsv", sep="\t")
...
# =============================================================================
# 3. STAR + htseq-count (extended GTF) -- already gene-level
# =============================================================================
print("\n=== STAR+htseq (extended GTF) ===")
star_ext = pd.read_csv("results/normalisation_star_htseq/deseq2_star_htseq_KO_vs_WT.tsv", sep="\t")
...
# =============================================================================
# 4. STAR + htseq-count (original GTF) -- already gene-level
# =============================================================================
print("\n=== STAR+htseq (original GTF) ===")
star_orig = pd.read_csv("results/normalisation_star_htseq_original_gtf/deseq2_star_htseq_KO_vs_WT.tsv", sep="\t")
