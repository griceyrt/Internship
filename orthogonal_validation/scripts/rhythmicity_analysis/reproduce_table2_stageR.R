#!/usr/bin/env Rscript
# Sanity check only -- faithfully reproduce the original transcript-level method
# (cosinor LRT + stageR two-stage) to confirm 501 is a real methodological
# difference (transcript-level+stageR vs the direct gene-level test), not an artifact.
# Author: Gricey
# Run from orthogonal_validation/: Rscript scripts/reproduce_table2_stageR.R
...
write.csv(circadian_analysis, paste(out_dir, "csv/reproduce_table2_stageR.csv", sep = ""), row.names = FALSE)

sig_genes <- unique(circadian_analysis$geneID[circadian_analysis$gene < fdr_cutoff])
length(sig_genes)  # compare to Table2/Fig1B's 501
