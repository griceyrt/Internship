...
# gene-level (blue) curves on the SAME panel per gene, full 656-gene list.
# Filtered version of ../fig1c_overlap_timecourse.R -- restricts the isoform
# side (tx_ids) to expressed transcripts (>5 reads in >=2 WT samples), same
# definition as tx_to_gene_rhythmicity.R (Khushi, 2026-08-20 email). The
# gene-level side is unaffected -- it's meant to reflect the gene's total
# expression regardless of any one isoform's status.
# Author: Gricey
# Run from orthogonal_validation/: Rscript scripts/rhythmicity_analysis/rhythmicity_filtered/fig1c_overlap_timecourse.R
...
## ---- same gene list as fig1c_isoform_timecourse.R (656 genes) ----
...
pdf(paste(out_dir, "figures/fig1c_overlap_timecourse.pdf", sep = ""), width = 10, height = 7.5)
