...
# gene-level (blue) curves on the SAME panel per gene, for the 43 "lost genes".
# Filtered version of ../fig1c_overlap_timecourse_lostgenes.R -- restricts the
...
# Author: Gricey
# Run from orthogonal_validation/: Rscript scripts/rhythmicity_analysis/rhythmicity_filtered/fig1c_overlap_timecourse_lostgenes.R
...
gene_list <- read.csv(paste(in_dir, "csv/lost_genes_list.csv", sep = ""))$gene_id
length(gene_list)  # 43
...
pdf(paste(out_dir, "figures/fig1c_overlap_timecourse_lostgenes.pdf", sep = ""), width = 10, height = 7.5)
