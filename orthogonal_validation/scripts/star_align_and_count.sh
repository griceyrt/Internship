...
# Aligns each of the 8 GSE130613 trimmed FASTQs with STAR (single-end),
# then counts reads per gene with htseq-count (union mode, unstranded --
# matching the -l U auto-detected by Salmon earlier in this project, and
# matching Asher's own "union mode" htseq-count usage).
#
# Reuses the already-trimmed FASTQs from temp/ (produced during the earlier
# Salmon runs) -- no need to re-download/re-trim.
...
OUTDIR="$BASE_DIR/results/SRP194523_star_htseq_$(date +"%Y-%m-%d")"
